	...
	.directive('phonenumberDirective', ['$filter', function($filter) {
		...
		*/
		...
		function link(scope, element, attributes) {
			...
			scope.phone_number = scope.phonenumberModel;

			scope.$watch('phone_number', function(value, oldValue) {
				
				value = String(value);
				var number = value.replace(/[^0-9]+/g, '');
				scope.phonenumberModel = number;
				scope.phone_number = $filter('phonenumber')(number);
			});
		}
		...
		return {
			...
			template: '<input ng-model="phone_number" required type="tel" class="form-control form-medium" placeholder="_ (___) _______" title="Phonenumber (Format: (999) 9999-9999)">',
		};
	}])
	    ...
	    */
	    ...
	    return function (number) {
		    ...
		    */
			...
			var c = (number[0] == '7') ? '7 ' : '';
			number = number[0] == '7' ? number.slice(1) : number;
